...
// */
//class ir(context: Context, attr: AttributeSet?=null, defStyleAttr:Int=0): View(context,attr,defStyleAttr) {
//    /*
...
class ir(context: Context, attr: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attr, defStyleAttr) {
    ...
    private var offsetX: Int = 50
    private var isAutomatic: Boolean = false
    val _isDownloadSuccessful:MutableStateFlow<String> = MutableStateFlow("null")
    val isDownloadSuccessful:StateFlow<String> = _isDownloadSuccessful
    init {
        mPaint = Paint()
        mPaint!!.strokeWidth = 20f
        mPaint!!.color = Color.BLACK
    ...
    fun downloadAndInitializeModel(languageTag: String): Boolean {
        return try {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                remoteModelManager.isModelDownloaded(rmodel).addOnSuccessListener { isDownloaded ->
                    if (!isDownloaded) {
                        remoteModelManager.download(rmodel, DownloadConditions.Builder().build())
                            .addOnSuccessListener {
                                Log.d("ModelDownload", "Model downloaded successfully.")
                                recognizer = DigitalInkRecognition.getClient(
                                    DigitalInkRecognizerOptions.builder(rmodel).build()
                                )
                                _isDownloadSuccessful.value="true"
                            }
                            .addOnFailureListener {
                                Log.e("ModelDownload", "Model download failed: $it")
                                _isDownloadSuccessful.value="false"
                            }
                    } else {
                        recognizer = DigitalInkRecognition.getClient(
                            DigitalInkRecognizerOptions.builder(rmodel).build()
                        ...
                        _isDownloadSuccessful.value="already"
                    }
                }
            }
    ...
    private fun recognize(ink: Ink) {
        recognizer.recognize(ink)
            .addOnSuccessListener { result ->
                Log.i("Recognition", result.candidates[0].text)
                val resultText = result.candidates[0].text
                val updatedResult = resultText.removePrefix(preContext)
                resultListener.getRecognitionResult(updatedResult)
                preContext = result.candidates[0].text
            }
            .addOnFailureListener {
                Log.e("Recognition", "Error during recognition: $it")
            }
    }

    fun offsetPaths(userOffset: Boolean) {
        for (path in paths!!.reversed()) {
            path.offset(if (!userOffset) -offsetX.toFloat() else -550f, 0f)
            invalidate()
        }
    }

    fun setRecognitionResultListener(listener: WorkShop.RecognitionResultListener) {
    ...
    fun setIsAutomatic(isAutomatic: Boolean) {
        this.isAutomatic = isAutomatic
    }
    fun getAvailableLanguageTags(): Array<String> {
        return DigitalInkRecognitionModelIdentifier.allModelIdentifiers()
            .mapNotNull { it.languageTag }
            .toTypedArray() // Converts the List<String> to an Array<String>
    }
